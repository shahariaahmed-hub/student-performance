...
# Columns that are never treated as a "subject" for averaging/charting
NON_SUBJECT_COLUMNS = ["Student_ID", "Attendance", "Average"]


def get_subject_columns(dataframe: pd.DataFrame) -> list:
    """Any numeric column that isn't an ID, Attendance, or the derived
    Average is treated as a subject. This means the app automatically
    supports any number of subjects (3, 6, or more) without code changes."""
...
if st.session_state.df is not None:
    ...
    with insights_tab:
        ...
        if not subject_columns:
            st.warning("No subject (numeric marks) columns were found in this file.")
        else:
            df["Average"] = df[subject_columns].mean(axis=1)
            st.session_state.df = df

            # -----------------------------
            # Average Marks distribution (own chart, not mixed with Attendance)
            # -----------------------------
            st.markdown('<p class="panel-title">Average Marks Distribution</p>', unsafe_allow_html=True)
            fig1, ax1 = plt.subplots(figsize=(10, 4))
            ax1.hist(df["Average"], bins=8, color=PALETTE[0], edgecolor="white")
            ...
            # -----------------------------
            # Attendance distribution (separate chart — no longer mixed in)
            # -----------------------------
            ...
            # -----------------------------
            # Average Marks by Subject (supports any number of subjects,
            # each bar gets its own color and a value label for clarity)
            # -----------------------------
            ...
            bars = ax3.bar(subject_averages.index, subject_averages.values,
